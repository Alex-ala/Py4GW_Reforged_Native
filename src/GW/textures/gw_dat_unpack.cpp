...
// Verbatim port of the legacy GwDatXentax decompressor. The tables and the
// Decompress worker are file-local; only UnpackGWDat is exported.
        ...
        do {
            ...
            if (_counter1 > -1)
                do {
                    ...
                    if (_temp == -1) {
                        int j = 0;
                        while (ESIplusC < HuffmanTree.HelperArray[j]) {
                            j += 3;
                        }

                        _temp = HuffmanTree.HelperArray[j + 2];
                        _data = HuffmanTree.HelperArray[j + 1] -
                            ((unsigned int)(ESIplusC - HuffmanTree.HelperArray[j]) >> (0x20 - _temp));
                    ...
                    if (_data < 0x100) {
                        ...
                        int k = (ESIplusC >> 0x18) * 2;
                        _temp = HuffmanTree2.HuffmanTable[k];
                        _data = HuffmanTree2.HuffmanTable[k + 1];

                        if (_temp == -1) {
                            int j = 0;
                            while (ESIplusC < HuffmanTree2.HelperArray[j]) {
                                j += 3;
                            }

                            _temp = HuffmanTree2.HelperArray[j + 2];
                            _data = HuffmanTree2.HelperArray[j + 1] -
                                ((unsigned int)(ESIplusC - HuffmanTree2.HelperArray[j]) >> (0x20 - _temp));
