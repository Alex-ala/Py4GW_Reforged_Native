...
void RenderDockspaceHost() {
    ...
    ImGui::Begin("Py4GW DockSpace", nullptr, dockspace_window_flags);
    ImGui::PopStyleVar(2);
    ...
    ImGui::DockSpace(ImGui::GetID("Py4GWDockSpaceRoot"), ImVec2(0.0f, 0.0f), dockspace_flags);
    ImGui::End();
}

void RenderOverlayWindows(bool* request_shutdown) {
    const ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    ...
    ImGui::Spacing();
    if (ImGui::Button("Shutdown Overlay")) {
        *request_shutdown = true;
    }
    ImGui::SameLine();
    ImGui::TextDisabled("Each addon is tracked in its own wrapper file.");
    ImGui::End();
    ...
    if (g_show_console) {
        py4gw::imgui::console_ui::Render(&g_show_console, &g_show_compact_console);
    }
    if (g_show_compact_console) {
        py4gw::imgui::compact_ui::Render(&g_show_console, &g_show_compact_console);
    }
}
...
void Render(bool* request_shutdown) {
    RenderDockspaceHost();
    RenderOverlayWindows(request_shutdown);
}
